...
Future<Stream<Product>> getTrendingProducts(Address address) async {
  ...
  _queryParams['search'] = 'active:1;';
  _queryParams['searchFields'] = 'active:=;';
...
Future<Stream<Product>> getFeaturedProducts(Address address) async {
  Uri uri = Helper.getUri('api/products');
  Map<String, dynamic> _queryParams = {};
  SharedPreferences prefs = await SharedPreferences.getInstance();
  Filter filter = Filter.fromJSON(json.decode(prefs.getString('filter') ?? '{}'));
  ...
  _queryParams['search'] = 'active:1;featured:1;';
  _queryParams['searchFields'] = 'active:=;featured:=;';
  if (!address.isUnknown()) {
    _queryParams['myLon'] = address.longitude.toString();
    _queryParams['myLat'] = address.latitude.toString();
    _queryParams['areaLon'] = address.longitude.toString();
...
Future<Stream<Product>> searchProducts(String search, Address address) async {
  Uri uri = Helper.getUri('api/products');
  Map<String, dynamic> _queryParams = {};
  _queryParams['search'] = 'market.name:$search;name:$search;description:$search;active:1;';
  _queryParams['searchFields'] = 'market.name:like;name:like;description:like;';
...
Future<Favorite> addFavorite(Favorite favorite) async {
  ...
  try {
    ...
    print("URL::${url}PARAM::${json.encode(favorite.toMap())}");
...
Future<Stream<Product>> getFeaturedProductsOfMarket(String marketId) async {
  ...
  uri = uri.replace(queryParameters: {
    'with': 'category;options;productReviews',
    'search': 'market_id:$marketId;featured:1;active:1;',
    'searchFields': 'market_id:=;featured:=;active:=;',
    'searchJoin': 'and',
  });
  try {
    final client = new http.Client();
