...
Future<Stream<Market>> getNearMarkets(Address myLocation, Address areaLocation) async {
  ...
  if (!myLocation.isUnknown() && !areaLocation.isUnknown() && filter.delivery) {
    ...
    _queryParams['search'] = 'closed:${filter.open ? 0 : 1};active:1;available_for_delivery:0;';
    _queryParams['searchFields'] = 'closed:=;active:=;available_for_delivery:=;';
  }
...
Future<Stream<Market>> searchMarkets(String search, Address address) async {
  Uri uri = Helper.getUri('api/markets');
  Map<String, dynamic> _queryParams = {};
  _queryParams['search'] = 'products.name:$search;name:$search;description:$search;';
  _queryParams['searchFields'] = 'products.name:like;name:like;description:like;';
