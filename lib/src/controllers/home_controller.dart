...
class HomeController extends ControllerMVC {
  ...
  Future<void> listenForFeaturedProducts() async {
    final Stream<Product> stream = await getFeaturedProducts(deliveryAddress.value);
    stream.listen((Product _product) {
  ...
  Future<void> requestForCurrentLocation(BuildContext context)async {
    OverlayEntry loader = Helper.overlayLoader(state.context);
    Overlay.of(state.context).insert(loader);
    setCurrentLocation().then((_address) async {
      deliveryAddress.value = _address;
      SharedPreferences prefs = await SharedPreferences.getInstance();
      var filter = Filter.fromJSON(json.decode(prefs.getString('filter') ?? '{}'));
      if(filter != null){
  ...
  Future<void> refreshHome() async {
    ...
    await listenForFeaturedProducts();
  }
}
