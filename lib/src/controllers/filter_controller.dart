...
class FilterController extends ControllerMVC {
  ...
  FilterController() {
    ...
    listenForFilter().whenComplete(() {
      if(filter != null && filter.fields.length == 0) {
        listenForFields();
      }else{
        if(filter != null) {
          setState(() {fields = filter.fields;});
        }
      }
    });
  }
  ...
  void resetFields() {
    filter.fields = [];
    fields.elementAt(0).selected = !fields.elementAt(0).selected;
    if(!fields.elementAt(0).selected){
